...
export function setupUI({game,network,voice}){
 const menu=document.getElementById("menu"),hud=document.getElementById("hud"),lobby=document.getElementById("roomLobby"),chars=document.getElementById("characters"),status=document.getElementById("menuStatus"),roomList=document.getElementById("roomList"),createPrivate=document.getElementById("createPrivate");
 ...
 function connect(room,extra={}){status.textContent=room==="CREATE"?"Creating Room…":room?"Joining Room…":"Finding Rooms…";network.connect({username:user(),character:selected,room,...extra})}
 ...
 document.getElementById("create").onclick=()=>connect("CREATE",{privateRoom:!!createPrivate?.checked,roomName:(name.value.trim()||"LUMI ROOM").slice(0,24)});
 ...
 const roomQ=new URLSearchParams(location.search).get("room");if(roomQ){connect(roomQ);menu.classList.add("hidden")}else{setTimeout(()=>menu.classList.remove("hidden"),700);network.connect({username:user(),character:selected,room:null})}
}
