...
export class RoomManager{
 ...
 create({name='LUMI ROOM',privateRoom=false}={}){let r=new Room(undefined,{name,private:!!privateRoom,permanent:false});while(this.rooms.has(r.code))r=new Room(undefined,{name,private:!!privateRoom});this.rooms.set(r.code,r);return r}
 get(c){return this.rooms.get(String(c||'').trim().toUpperCase())}
 join(c,p,opts={}){let r;if(String(c).toUpperCase()==='CREATE')r=this.create(opts);else if(c)r=this.get(c);else return null;if(!r||r.players.size>=20)return null;r.add(p);return r}
